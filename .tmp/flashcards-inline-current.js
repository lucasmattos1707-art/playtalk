(() => {
      ...
      const FLASHCARDS_R2_SOURCE_PREFIX = 'Star';
      const FLASHCARDS_R2_SOURCE_ROOT = `${FLASHCARDS_R2_PUBLIC_ROOT}/${FLASHCARDS_R2_SOURCE_PREFIX}`;
      ...
      const DATA_MANIFEST_REMOTE_PATH = `${FLASHCARDS_R2_SOURCE_ROOT}/manifest.json`;
      const FLASHCARDS_SOURCE_TREE_API_PATH = `/api/r2/storage-tree?prefix=${encodeURIComponent(`${FLASHCARDS_R2_SOURCE_PREFIX}/`)}`;
      ...
      function setAuthMode(mode) {
        ...
        els.authCopy.textContent = isRegister
          ...
          : 'Entre com seu nome e senha para começar a jogar na Liga Playtalk.';
      }
      ...
      async function handleLoggedOutState(message) {
        ...
        els.allSectionCopy.textContent = 'Entre ou crie seu nome para começar.';
      ...
      function buildFlashcardsPublicUrl(objectKey) {
        ...
        return encodedKey ? `${FLASHCARDS_R2_PUBLIC_ROOT}/${encodedKey}` : '';
      }
      ...
      async function fetchDeckFilesFromSourcePrefix() {
        const response = await fetch(withNoCacheUrl(FLASHCARDS_SOURCE_TREE_API_PATH), { cache: 'no-store' });
        if (!response.ok) {
          throw new Error('Nao consegui listar os decks de flashcards no R2.');
        }
        ...
        return keys
          .filter((key) => (
            safeText(key).startsWith(`${FLASHCARDS_R2_SOURCE_PREFIX}/`)
            && safeText(key).toLowerCase().endsWith('.json')
            && !/\/manifest\.json$/i.test(safeText(key))
          ))
          .sort((left, right) => left.localeCompare(right, 'pt-BR', { sensitivity: 'base', numeric: true }))
          .map((key) => ({
            name: safeText(key).split('/').pop() || key,
            path: buildFlashcardsPublicUrl(key),
            sourceKey: key,
            idSource: key
          }));
      }
      ...
      async function refreshCardsFromNetwork() {
        ...
        const files = manifestFiles.length
          ...
          : await fetchDeckFilesFromSourcePrefix();
        const responses = await Promise.all(files.map(async (file) => {
          const filePath = normalizeFlashcardsDataPath(file.path);
          if (!filePath) {
            return { cards: [], deck: buildDeckCatalogEntry(file, null, []) };
          }
      ...
      function refreshLibrary() {
        ...
        els.allSectionCopy.textContent = cards.length
          ? adminMode
            ? `${cards.length} flashcards${username} disponiveis no catalogo completo.`
        ...
        renderMiniCards(
          ...
          visibleCards,
          adminMode
            ? 'Nenhum flashcard disponivel no catalogo ainda.'
